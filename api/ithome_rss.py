...
class ITHomeRSS(BaseAPI):
    ...
    async def get_rss_async(self) -> Optional[ET.Element]:
        ...
        try:
            ...
            async with session.get(
                self.url, headers=self.headers, timeout=aiohttp.ClientTimeout(total=10)
            ) as response:
                ...
                if len(content) > max_size:
                    logger.warning(
                        f"RSS内容过大 ({len(content)} bytes)，已截断至 {max_size} bytes"
                    )
    ...
    def parse_news(
        self, rss_root: Optional[ET.Element], max_count: int = 5
    ) -> List[str]:
        """
        解析 RSS 数据，提取新闻标题
