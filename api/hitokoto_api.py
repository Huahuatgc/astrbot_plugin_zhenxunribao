...
class HitokotoAPI(BaseAPI):
    ...
    def _get_default_hitokoto(self) -> Dict[str, str]:
        return {"hitokoto": "生活就像骑自行车，想保持平衡就得往前走。", "from": "未知"}
        ...
        try:
            ...
            async with session.get(
                ...
                if (code == 200 or success) and data.get("data"):
                    ...
                    # 获取from字段
                    from_value = (
                        hitokoto_data.get("from") or hitokoto_data.get("from_who") or ""
                    )

                    # 如果为空或"网络"则使用"佚名"
                    if not from_value or (
                        isinstance(from_value, str)
                        and (from_value.strip() == "" or from_value.strip() == "网络")
                    ):
                        from_value = "佚名"
                    else:
                    ...
                    logger.warning(
                        f"API返回异常: code={code}, success={success}, message={data.get('message', '未知错误')}"
                    )
